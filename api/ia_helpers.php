...
const IA_MODELO_RESUMEN_HOY = 'claude-haiku-4-5-20251001';

// Convierte el listado de pendientes/vencimientos que el frontend ya
// calculó (buildAgendaEntries() en app.js -- fechas y días ya resueltos
// ahí, con la misma lógica que usan la Agenda general y las alertas de
// prescripción) en un texto corto y priorizado para que un coordinador o
// un abogado sepa qué atender primero al abrir el sistema. La IA solo
// organiza y redacta -- nunca calcula ni inventa una fecha, un plazo, un
// artículo o un monto que no venga tal cual en $items.
function ia_generar_resumen_hoy(array $items): ?string
{
    $credentialsFile = __DIR__ . '/anthropic_credentials.php';
    if (!file_exists($credentialsFile)) {
        error_log('Falta api/anthropic_credentials.php');
    ...
    $lineas = [];
    foreach ($items as $it) {
        $caso = trim(($it['actor'] ?? '') . (!empty($it['demandado']) ? ' vs ' . $it['demandado'] : ''));
        $dias = isset($it['dias']) && $it['dias'] !== null ? (int)$it['dias'] : null;
        $cuando = $dias === null ? '' : ($dias < 0 ? ' -- VENCIDO hace ' . abs($dias) . ' día(s)' : ($dias === 0 ? ' -- HOY' : ' -- en ' . $dias . ' día(s)'));
        $lineas[] = '- [' . ($it['tipo'] ?? 'pendiente') . '] ' . ($caso !== '' ? $caso . ': ' : '') . ($it['label'] ?? '') . $cuando;
    }
    if (!$lineas) return null;

    $payload = [
        'model' => IA_MODELO_RESUMEN_HOY,
        'max_tokens' => 500,
        'thinking' => ['type' => 'disabled'],
        'system' => 'Eres el asistente interno de un despacho de derecho laboral en México. Te doy una lista de '
            . 'pendientes, vencimientos y alertas de varios expedientes, cada uno con cuántos días faltan o ya se '
            . 'vencieron -- estos datos YA están calculados, tu única tarea es organizarlos y redactar un "Qué '
            . 'hacer hoy" corto y priorizado para quien abre el sistema en la mañana (puede ser el coordinador del '
            . 'despacho viendo todos los casos, o un abogado viendo solo los suyos).'
            . "\n\n"
            . 'Formato: agrupa en como máximo 3 bloques por urgencia -- "Urgente / vencido", "Esta semana", "Sin '
            . 'prisa, dar seguimiento" -- omite cualquier bloque que quede vacío. Dentro de cada bloque, una línea '
            . 'por asunto con un guion, mencionando el actor/caso y qué hay que hacer, en tono directo y breve '
            . '(estilo lista de pendientes, no párrafos). Máximo 12 líneas de contenido en total -- si hay más de '
            . '12 elementos, incluye los más urgentes primero y cierra con una línea tipo "+ N pendientes más de '
            . 'menor urgencia, revisa la Agenda general."'
            . "\n\n"
            . 'REGLA DURA: nunca inventes ni cambies una fecha, un plazo, un artículo de ley o un monto que no '
            . 'venga tal cual en los datos que te doy -- solo organiza y prioriza lo que ya está aquí. No agregues '
            . 'ningún dato nuevo, ninguna interpretación legal, ni sugerencias de qué haría falta que no esté ya '
            . 'en la lista.'
            . "\n\n"
            . 'No uses markdown con doble asterisco ni encabezados con #; para negritas usa mayúsculas o nada. '
            . 'Español de México, tono profesional y directo.',
        'messages' => [['role' => 'user', 'content' => implode("\n", $lineas)]],
    ];
    ...
    if ($raw === false || $status !== 200) {
        file_put_contents(__DIR__ . '/ia_debug.log', date('c')
            . " | [resumen_hoy] status=$status | curl=$curlError | body=" . (string)$raw . "\n", FILE_APPEND);
        return null;
    }
    ...
    $u = $data['usage'] ?? [];
    file_put_contents(__DIR__ . '/ia_debug.log', date('c')
        . " | [resumen_hoy] input=" . ($u['input_tokens'] ?? 0)
        . " | output=" . ($u['output_tokens'] ?? 0) . "\n", FILE_APPEND);
